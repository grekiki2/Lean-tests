...
\title{Verification of the chromatic number of a graph with the Lean proof assistant}
...
2-obarvljivosti v polinomskem času.}
...
V tej nalogi se ukvarjamo z enostavnimi končnimi neusmerjenimi grafi, ker je cilj preverjati lastnosti za konkretne grafe ki jih
lahko predstavimo v računalniku.
\begin{definicija}
...
\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.15]{assets/graph}
...
Če je $G$ graf in obstaja $k$-barvanje grafa $G$, pravimo, da je $G$ $k$-obarvljiv.
\begin{definicija}
...
\begin{proof}
    ...
    grafa $G$ enako najmanjšemu elementu $C_G$. Dovolj je dokazati, da je $C_H\subset C_G$, ker je minimum podmnožice vedno
...
\begin{lema}
    Naj bo $f:V(G)\to [k1]$ barvanje grafa $G$ in $k2>k1$. Potem obstaja barvanje $f':V(G)\to [k2]$ grafa $G$.
\end{lema}
\begin{proof}
    Dovolj je definirati $f'$ kot $f'(v) = f(v)$ za vsa vozlišča $v\in V(G)$. To je ustrezna definicija, ker je $k2>k1$.
\end{proof}

\begin{lema}
    Graf $G$ je $k$-obarvljiv če in samo če je kromatično število grafa $G$ manjše ali enako $k$.
\end{lema}
\begin{proof}
    Najprej dokažimo implikacijo v desno. Kromatično število smo definirali kot najmanjši $i$, da je graf $G$ $i$-obarvljiv.
    Če je torej graf $G$ $k$-obarvljiv, potem je kromatično število grafa $G$ največ $k$.
    V levo če vemo, da je kromatično število grafa $G$ največ $k$, potem obstaja barvanje grafa $G$ z največ $k$ barvami
    ki ga lahko razširimo do barvanja s $k$ barvami po lemi 
\end{proof}

\subsection{Kromatična števila cikličnih grafov}
\begin{figure}[H]
    \begin{center}
    \includegraphics[scale=0.5]{assets/C_n}
    \caption{Primeri cikličnih grafov}
    \label{slika_Cn}
    \end{center}
\end{figure}
\begin{definicija}
    Ciklični graf $C_n$ kjer je $n\geq 3$ je graf z $n$ vozlišči, kjer sta vozlišči $i$ in $j$ povezani
    če in samo če je $|i-j|= 1$ ali pa velja $i=0$ in $j=n-1$ ali $j=0$ in $i=n-1$.
\end{definicija}
Da lažje primerjamo definicjo s sliko dokažimo naslednjo lemo. 
\begin{lema}
    Vsako vozlišče cikličnega grafa $C_n$ ima stopnjo 2.
\end{lema}
\begin{proof}
    Denimo da je $0<i<n-1$. Potem so edine rešitve za $j$ enačbe $|i-j|=1$ enake $i-1$ in $i+1$. Torej je stopnja
    res 2. Za $i=0$ sta edini možnosti $j=1$ kot rešitev enačbe in $j=n-1$ po drugem delu definicije. 
    Za $i=n-1$ je edina veljavna rešitev enačbe $j=n-2$, drugo sosednje vozlišče pa je po definiciji $j=0$.
\end{proof}

Pri naslednjih dveh dokazih so določene podrobnosti prepuščene bralcu.
\begin{lema}
    Če je $n$ sodo število, je kromatično število cikličnega grafa $C_n$ enako 2.
\end{lema}
\begin{proof}
    Dokažimo da obstaja barvanje z dvema barvama. Barvanje definiramo kot $f(i) = i\mod 2$.
    Z razdelitvija na primere se lahko prepričamo da je to barvanje veljavno. 
    Ker ima graf $C_n$ vsaj eno povezavo, barvanje z $1$ barvno ne obstaja.
    Torej je kromatično število grafa $C_n$ za sod $n$ enako 2.
\end{proof}

\begin{lema}
    Če je $n$ liho število, je kromatično število cikličnega grafa $C_n$ enako 3.
\end{lema}
\begin{proof}
    Dokažimo da obstaja barvanje z tremi barvami. Barvanje definiramo kot $f(i) = i\mod 2$ če $i \neq 0$ in $f(0) = 2$.
    Z razdelitvijo na primere se lahko prepričamo da je to barvanje veljavno.

    Dokažimo da barvanje z dvema barvama ne obstaja. Brez škode za splošnost je vozlišče $0$ pobarvano z barvo $0$.
    ...
    Poglejmo si sedaj vozlišči $0$ in $2n$. Po indukcijski predpostavki sta obe pobarvani z barvo $0$, 
    vendar pa sta povezani, kar je protislovje. Torej 2 barvanje ne obstaja.
\end{proof}
...
Med številnimi dokazovalnimi pomočniki, kot so Coq, Agda in Isabelle, za naše delo izberemo Lean iz več razlogov, predvsem
zaradi enostavnosti uporabe, aktivne skupnosti in primernosti za dokazovanje pravilnosti algoritmov. 

Lean je interaktivni dokazovalni pomočnik. Uporabljamo verzijo Lean 4, ki je razvita pri Microsoftu in temelji na teoriji tipov
s čimer se ne ukvarjamo, ker je tema te diplomske naloge teorija grafov. Lean 4 uporabljamo tudi
...
Osnovni tipi, s katerimi se največ ukvarjamo, so:
\begin{itemize}
...
#eval 2
\end{lstlisting}
se izračuna v 2, Lean pa nam pove, da je tip izraza \lstinline{Nat}. Če pa želimo izraz izračunati kot \lstinline{Int}, lahko to naredimo z naslednjim ukazom.
\begin{lstlisting}
#eval (2 : Int)
\end{lstlisting}
To nam potem predstavlja 2 kot celo število. Lean ponavadi namesto
...
def f (x : ℕ) : ℕ := x + 1
\end{lstlisting}
Definiramo funkcijo \lstinline{f}, ki sprejme naravno število in vrne naravno število. Funkcijo lahko poračunamo podobno kot izraze.
\begin{lstlisting}
#eval f 2
\end{lstlisting}
Izraz se izračuna v 3. Če nismo prepričani kakšen tip ima nek objekt v Leanu lahko to ugotovimo z naslednjim ukazom.
\begin{lstlisting}
#check f
\end{lstlisting}
Ukaz nam vrne \lstinline{ℕ → ℕ}, torej je funkcija $f$ preslikava iz naravnih števil v naravna števila.

Lahko definiramo tudi funkcije več spremenljivk.
\begin{lstlisting}
def vsota (x: ℕ) (y: ℕ) : ℕ := x + y
\end{lstlisting}
Funkcija ima tip \lstinline{ℕ → ℕ → ℕ}, torej je preslikava iz naravnih števil v preslikave iz naravnih števil v naravna števila.
...
def funkcija4 (a:Nat) : Nat :=
    if a = 0 then
      ...
      1 + funkcija4 (a-1)
\end{lstlisting}
Pri rekurziji moramo poskrbeti, da se izvajanje zaključi. Včasih Lean pri tem potrebuje nekaj pomoči, detajli o tem so izven obsega te diplomske naloge.

Stavke \lstinline{if}, rekurzijo in pomožne spremenljivke uporabljamo tudi v funkcijah več spremenljivk.
\begin{lstlisting}
...
#eval izhodisce.x
\end{lstlisting}
Do posameznih komponent torej dostopamo z \lstinline{ime_objekta.ime_komponente}.
...
Vidimo, da lahko podobno kot v OCamlu uporabljamo \lstinline{_} v match stavku da pokrijemo vse možnosti, ki še niso bile pokrite.
...
Poglejmo si definicijo kromatičnega števila v Leanu.
Za berljivost bomo večino dokazov spuščali.
Celotna koda je dostopna na \cite{leanG}.
...
Definicija grafa, ki smo jo navedli, je precej splošna in jo lahko uporabimo tudi za definicije grafov iz seznama povezav.
...
\end{lstlisting}
Podobno bi lahko naredili tudi z grafom, ki bi za seznam sosednosti uporabljal tip \lstinline{List}, vendar pa je uporaba \lstinline{Array} 
bolj učinkovita.
...
Predstavimo $k$-barvanje grafa kot preslikavo iz (indeksa) vozlišča v eno izmed $k$ barv.
...
je napisati in pričakovati da Lean poračuna naslednji izraz.
\begin{lstlisting}
#eval valid_coloring (K_n 5) (λ x => 0)
\end{lstlisting}
Lean na žalost izračunljivosti funkcije \lstinline{valid_coloring} ne ugotovi sam, zato moramo dokazati,
da je \lstinline{valid_coloring} izračunljiva funkcija oziroma v Leanu \lstinline|Decidable|.
\begin{lstlisting}
...
\lstinline{is_k_colorable G k}. V Leanu to naredimo s pomočjo funkcije \lstinline{Nat.find},
...
Definicija kromatičnega števila v Leanu je potem naslednja.
\begin{lstlisting}
...
\end{lstlisting}
Vzamemo torej najmanjše število barv, da je graf $G$ $k$-obarvljiv, zraven pa dokažemo, da vsaj eno tako število obstaja.
...
kromatičnega števila. Nasledji primeri so izračuni kromatičnega števila za nekaj preprostih grafov.
\begin{lstlisting}
...
\end{lstlisting}
Prvi primer nam za graf $K_8$ vrne 8, kar je pričakovano, saj je $K_8$ poln graf in je kromatično število enako številu vozlišč, 
drugi in tretji primer pa vrneta $3$ in $2$, kar je prav tako pričakovano, saj smo v prejšnjem poglavju pokazali, 
da je kromatično število cikličnih grafov enako $3$ za liho število in $2$ za sodo število vozlišč.
Za računanje kromatičnega števila seveda obstajo hitrejši algoritmi, kot je naša definicija, 
ki deluje tako, da preveri vsa možna barvanja pri nekem $k$, dokler ne najde veljavnega ali pa gre 
na naslednji $k$, vendar pa je dokazovanje pravilnosti kompleksnejših algoritmov precej zahtevno.
...
\end{lstlisting}
Nato lahko, podobno kot smo to naredili v 2. poglavju, dokažemo, da $k$-barvanje grafa $G$ obstaja, če in samo če je kromatično število grafa $G$ največ $k$.
\begin{lstlisting}
lemma colorable_gives_ub (G: Graph) (k: Nat):
    is_k_colorable G k ↔ chromatic_number G ≤ k
\end{lstlisting}

Posledica je, da barvanje s $k$ barvami ne obstaja če in samo če je $k$ manjši od kromatičnega števila grafa $G$.
\begin{lstlisting}
...
Če najdemo nek $k$, da graf $G$ ni $k$-obarvljiv, je pa $(k+1)$-obarvljiv, potem je kromatično število grafa $G$ enako $k+1$.
\begin{lstlisting}
theorem bounds_give_chromatic (G:Graph) (k: Nat):
    ¬is_k_colorable G k ∧ is_k_colorable G (k+1) ↔ chromatic_number G = k+1
\end{lstlisting}

Če sedaj na dokazano gledamo iz vidika uporabnosti, je precej preprosto Leanu razložiti, da je graf $G$ $k$-obarvljiv
recimo tako, da najdemo neko barvanje. Večji problem pa je s spodnjo mejo kromatičnega števila. 
V tem delu se spodnje meje lotimo s pomočjo podgrafov.
...
ali pa $C_n$ ciklični grafi, ki imajo kromatično število $2$, če je $n$ sodo in $3$, če je $n$ liho število kot smo dokazali v 2. poglavju. 

\subsection{2-obarvljivost}
Hitri algoritmi za računanje kromatičnega števila v splošnem niso znani, 
ve se, da je na primer problem določiti ali je graf $3$-obarvljiv, NP poln.
Za $2$-obarvljivost pa hiter algoritem obstaja in ga tudi implementiramo v Leanu ter dokažemo njegovo pravilnost. 
Naš trenuten algoritem porabi eksponentno količino časa glede na število vozlišč, ker mora pregledati vsa možna barvanja.

Implementacija hitrejše verzije je dostopna na \cite{leanG}, tukaj pa
opišemo princip delovanja. 
...
Če barvanje vseh vozlišč uspe, potem vrnemo barvanje, ki ga Lean lahko preveri in s tem dokaže, da je graf $2$-obarvljiv.
...
vemo, da ima kromatično število $3$, to pa nam pove, da graf ni $2$-obarvljiv.
...
S tem dobimo hiter in pravilen algoritem za določanje $2$-obarvljivost grafa, je pa res, da smo uspeli dokazati le pravilnost
ne pa tudi hitrosti.


\section{Rezultati in meritve}
V tem poglavju predstavimo rezultate in meritve.
Ker kromatično število vedno računamo s tem, da gledamo, ali je graf $k$-obarvljiv, bodo tudi ocene hitrosti zaradi enostavnosti
primerjale hitrosti implementacij $k$-obarvljivosti za različne $k$ in različne algoritme, ki smo jih implementirali. 

Poglejmo si najprej osnovni algoritem za preverjanje $k$-obarvljivosti.
\begin{lstlisting}
def is_k_colorable (G : Graph) (k: Nat) : Prop :=
    ∃ (coloring : Coloring G k), valid_coloring G coloring
\end{lstlisting}

Obarvljivost torej preverimo tako, da preverimo, ali izmed vseh mogočih barvanj obstaja barvanje z $k$ barvami.

Poglejmo si nekaj grafov porabe časa tega algoritma. Za testiranje algoritma bomo naključno generirali grafe na $n$ vozliščih
kjer bo vsaka povezava prisotna z verjetnostjo $p$. Označimo tak tip grafov z $G(n, p)$.

\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.35]{assets/2barvSlow}
\caption{Poraba časa algoritma za preverjanje 2-obarvljivosti grafa $G(n, p)$}
\label{slika3}
\end{center}
\end{figure}
...
\caption{Poraba časa algoritma za preverjanje 3-obarvljivosti grafa $G(n, p)$}
\label{slika4}
\end{center}
\end{figure}
...
\caption{Poraba časa algoritma za preverjanje 5-obarvljivosti grafa $G(n, p)$}
\label{slika5}
\end{center}
\end{figure}

Grafe smo pridobili tako, da je Python klical prevejeno Lean kodo in beležil čas izvajanja. Program v Leanu je zgeneriral 1000
naključnih grafov $G(n, p)$ ter za vsakega preveril, ali je $k$-obarvljiv, kjer je bil $k$ podan kot argument programa.
...
Vidimo, da je poraba časa eksponentna v številu vozlišč grafa ter da je preverjanje $k$-obarvljivosti za večje $k$ 
...
Vidimo, da je za preverjanje $3$ in $5$-obarvljivosti algoritem počasnejši, več kot je povezav, kar je smiselno, ker je pri velikem 
številu povezav verjetnost, da barvanje obstaja manjše, zato program ne more ustaviti izvajanja, ko najde veljavno barvanje. 
Pri $2$-obarvljivosti ta opazka o hitrosti iz neznanih razlogov ne velja. 

Na podoben način preizkusimo tudi hitrejši algoritem za preverjanje $2$-obarvljivosti.
\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.4]{assets/2barvFast}
\caption{Poraba časa hitrega algoritma za preverjanje 2-obarvljivosti grafa $G(n, p)$}
\label{slika6}
\end{center}
\end{figure}

Vidimo da je algoritem za katerega smo trdili, da je hitrejši brez dokaza, (in dokazali pravilnost), res bistveno hitrejši.
Pomembno je tudi, da je tukaj $y$ os krajša kot v prejšnjih grafih.
Za primerjavo različnih podatkovnih struktur enak algoritem poženemo tudi na leanovem tipu \lstinline{List} ki pa je osnovan
na strukturi povezanega seznama in zato pričakovano počasnejši.

\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.4]{assets/2barvFastList}
\caption{Poraba časa hitrega algoritma za preverjanje 2-obarvljivosti grafa $G(n, p)$ z uporabo \lstinline{List}}
\label{slika7}
\end{center}
\end{figure}
...
\section{Zaključek}
Ugotovimo, da je Lean 4 primeren jezik za implementacijo in dokazovanje pravilnosti algoritmov za računanje kromatičnega
